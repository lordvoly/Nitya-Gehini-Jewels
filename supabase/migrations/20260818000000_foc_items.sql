-- booking_items.is_foc: per-item Free-of-Cost flag, restricted to bookings
-- whose customer is MUA/Influencer (enforced server-side in bookings.ts,
-- not just hidden in the UI — see the route handlers). Deliberately does
-- NOT zero out or overwrite price_charged — the real listed price stays
-- stored as a reference of what the item would have cost; every place
-- that computes an effective charge treats is_foc = true as contributing
-- ₹0 instead (see reportsData.ts's effectivePrice()).
alter table booking_items add column is_foc boolean not null default false;

-- booking_financials: the one view every total_paid/balance_due figure in
-- this app is ultimately derived from (get_outstanding_dues, the
-- Dashboard's outstanding-balance card, BookingDetail, receipts) — only
-- the price subquery changes (is_foc treated as 0); the view's own output
-- columns (booking_id/price_charged/total_paid/balance_due) are
-- unchanged, so `create or replace view` is legal here, no drop needed.
create or replace view booking_financials as
select
  b.id as booking_id,
...
from bookings b
left join (
  select booking_id, sum(case when is_foc then 0 else price_charged end) as total_price
  from booking_items
...
-- create_booking_with_items: same 9-arg signature as
-- 20260816000000_booking_date.sql's version — is_foc travels inside each
-- element of the existing p_items jsonb array, not a new top-level
-- parameter, so a plain `create or replace` is enough (a drop is only
-- needed when the argument LIST itself changes, per that migration's own
-- note). Only change from the previous version: is_foc is read from each
-- item and stored on the new column.
